...
template <typename _Datatype> struct RingBuffer
{
public:
    ...
    {
        return Eigen::Vector3i(idx2add(id.x(), size_x - 1), idx2add(id.y(), size_y - 1), idx2add(id.z(), size_z - 1));
    }
...
public:
    inline void setup(const double& res, const Eigen::Vector3d& map_size, const double& cam_range, bool use_global_map = false)
    {
        resolution = res;
    ...
    inline void setupP(const int& _p_min, const int& _p_max, const int& _p_hit, const int& _p_mis, const int& _p_occ, const int& _p_def)
    {
    ...
    {
        return !((id.x() - offset_x) & (~(size_x - 1))) && !((id.y() - offset_y) & (~(size_y - 1))) && !((id.z() - offset_z) & (~(size_z - 1)));
    }
    ...
    {
        return id.x() - offset_x >= inflate_size && id.x() - offset_x < size_x - inflate_size && id.y() - offset_y >= inflate_size
               && id.y() - offset_y < size_y - inflate_size && id.z() - offset_z >= inflate_size && id.z() - offset_z < size_z - inflate_size;
    }
    inline const bool isInSmallMap(const Eigen::Vector3d& p) const
    {
        return isInSmallMap(pos2idx(p));
    }
    template <typename _Msgtype> inline void to_msg(_Msgtype& msg)
    {
    ...
    template <typename _Msgtype> inline void from_msg(const _Msgtype& msg)
    {
...
private:
    ...
    inline bool filter(const Eigen::Vector3d& sensor_p, const Eigen::Vector3d& p, Eigen::Vector3d& pt) const
    {
...
public:
    ...
    void updateMap(const Eigen::Vector3d& sensor_p, const std::vector<Eigen::Vector3d>& pc);
